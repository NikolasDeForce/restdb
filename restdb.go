...
func InsertUser(u User) bool {
	...
	if IsUserValid(u) {
		log.Println("User", u.Username, "already exist!")
		return false
	}
...
// ListAllUsers if for returning all users from the database table
...
// FindUserID if for returning a user record defined by ID
...
// Same as on top, returns user record by username
...
// IsUserAdmin return bollian administration or not
